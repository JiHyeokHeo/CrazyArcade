...
namespace ty
{
	...
	void ToyStage1::Initialize()
	{
		...
		mDao->GetComponent<Transform>()->SetPos(Vector2(380.0f, 700.0f));
	...
	void ToyStage1::Update()
	{
		...
		CheckCharactor::Skip(eSceneType::ToyStage2); // Q누르면 스킵
		Scene::Update();
	}
	...
	void ToyStage1::OnExit()
	{
		...
		if (SceneManager::GetIsDuo() == false)
		{
			...
			{
				mDao->GetComponent<Transform>()->SetPos(Vector2(380.0f, 700.0f)); // 화면 전환시 기능 추가
